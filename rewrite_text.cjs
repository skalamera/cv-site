...
async function rewriteText() {
  const prompt = `Rewrite the following text to strictly follow the WRITING RULES below. Make it direct, specific, and natural. Remove any bloat, puffery, and machine-sounding phrases. Do not use words like 'elite', 'stabilize', 'protect', 'alignment', or 'rapid triage' unless they are literal. Keep the specific numbers and technologies. Make it punchy and human.

TEXT TO REWRITE:
In my leadership roles at Sigma and Benchmark Education Company, I have been accountable for maintaining elite service standards, including a 4.84/5 CSAT and a 23-second average live-chat first response time. To ensure these SLAs were consistently met, I developed a Python-based ML forecasting app to predict ticket volumes and built real-time Power BI operational dashboards to stabilize backlogs and protect queue health. By automating workflows and API integrations, I successfully reduced resolution times by 38% and improved first response times by 45%, while personally owning SEV-0/SEV-1 incident escalations to ensure rapid triage and stakeholder alignment
